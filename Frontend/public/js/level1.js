...
function winner() {
	...
	var level_id = level;
	level = level + 1;
	var description = `Anda telah memainkan Level 1 dengan score ${score}`;
...
function update () {
	...
	if (snake.lives.length == 0 && live < 3) {
		const randPoss = {
			x: Math.floor(Math.random() * row),
			y: Math.floor(Math.random() * col)
		}
	...
	var eated = false;
	const gulp = new Audio('/js/gulp.mp3');
	...
	const run = new Audio('/js/run.wav');
	...
	const livePoint = new Audio('/js/livePoint.wav');
